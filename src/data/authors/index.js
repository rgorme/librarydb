...
const register = async ( { sql, getConnection } ) => {
   ...
   const addAuthor = async ( { FIRST_NAME, LAST_NAME, NATIONALITY_SHORT, DESCRIPTION } ) => {
        const pool = await getConnection();
        const request = await pool.request();
   ...
   const updateAuthor = async ( { FIRST_NAME, LAST_NAME, NATIONALITY_SHORT, DESCRIPTION, AUTHOR_ID } ) => {
        const pool = await getConnection();
        const request = await pool.request();
        request.input( "FIRST_NAME", sql.VarChar(250), FIRST_NAME);
        request.input( "LAST_NAME", sql.VarChar(250), LAST_NAME);
